...
def add_to_fav(request):
    ...
    fav_product = Favourite.objects.filter(user=request.user, piece=piece_obj, status=True).last()
    if fav_product:
        fav_product.status = False
        fav_product.save()
    else:
        fav_product = Favourite.objects.filter(user=request.user, piece=piece_obj).last()
        if fav_product:
